...
class Grade(models.Model):
    ...
    def _greater_or_equal_zero_sales_allowance(self):
        for record in self:
            if any(allowance < 0 or allowance > 100 for allowance in [record.sales_bonus_entry_rate,record.sales_bonus_senior_rate,record.sales_bonus_specialist_rate,record.sales_bonus_manager_rate,]):
                        raise ValidationError("The following allowances must be greater than or equal to zero:\n"
                                              "- Sales Bonus Entry\n"
                                              "- Sales Bonus Senior Entry\n"
                                              "- Sales Bonus Specialist\n"
    ...
    def _greater_or_equal_zero_remote_allowance(self):
        for record in self:
            if any(allowance < 0 for allowance in [record.travel_allowance,record.food_allowance,record.device_allowance,]):
                raise ValidationError("The following allowances must be greater than or equal to zero:\n"
                                                "- Travel Allowance\n"
                                                "- Food Allowance\n"
                                                "- Device Allowance\n"
    ...
    def _greater_or_equal_zero_loyal_allowance(self):
        for record in self:
            if any(allowance < 0 for allowance in [record.loyality_allowance_Entry,record.loyality_allowance_senior,record.loyality_allowance_specialist,]):
                raise ValidationError("The following allowances must be greater than or equal to zero:\n"
                                        "- Loyalty allowance Entry\n"
                                        "- Loyalty allowance senior\n"
                                        "- Loyalty allowance specialist\n"
